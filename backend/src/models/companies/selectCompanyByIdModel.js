...
const selectCompanyByIdModel = async (companyId) => {
    ...
    try {
        ...
        const [companies] = await connection.query(
            `SELECT C.id, C.name, C.country, C.city, C.bio, C.photo, C.userId, C.createdAt, ((avg(RC.salary) + avg(RC.workEnvironment) + avg(RC.promotionPosibility) + avg(RC.accesibility)) / 4) AS totalAvgRatings, RC.review, E.userId AS employeeId, E.confirmed FROM companies C 
