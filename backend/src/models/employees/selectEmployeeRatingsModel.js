...
const selectEmployeeRatingModel = async (userId) => {
    ...
    try {
        ...
        const [ratings] = await connection.query(
            `SELECT RC.id AS uniqueKey, RC.salary AS avgSalary, RC.workEnvironment AS avgWorkEnvironment, RC.promotionPosibility AS avgPromotionPosibility, RC.accesibility AS avgAccesibility, RC.review, RC.companyId, RC.userId, RC.createdAt, C.name, C.city, C.country, C.bio, C.photo 
